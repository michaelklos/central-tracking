/**
 * Loopback HTTP client to the ct server. Plugins instantiate this with their
 * own pluginId; shared across all Central Tracking plugins so the
 * env-shadowing / secret-masking semantics stay consistent.
 */
...
export class CtClient {
  ...
  constructor(pluginId: string, opts: CtClientOptions = {}) {
    if (!pluginId) throw new Error('CtClient: pluginId is required');
    const baseURL = opts.baseUrl ?? process.env.CT_SERVER_URL;
    const token = opts.token ?? process.env.CT_TOKEN;
    if (!baseURL) throw new Error(`CT_SERVER_URL not set (run via \`ct plugin run ${pluginId}\`)`);
    if (!token) throw new Error(`CT_TOKEN not set (run via \`ct plugin run ${pluginId}\`)`);
   ...
   * shadows the DB value — the recommended way to source secrets from a
   * vault / password manager instead of storing them in the SQLite db.
   *
   * Non-secret keys are NOT env-overridable.
   */
  ...
  getPendingSyncComments(pluginId: string): Promise<CtPendingSyncComment[]> {
    return this.call('comments/getPendingSync', [pluginId]);
  }
