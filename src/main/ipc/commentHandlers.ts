...
export function registerCommentHandlers(ipcMain: IpcMain, db: Database): void {
  ipcMain.handle('comments:getByTask', (_event, taskId: string) => {
  ...
  ipcMain.handle('comments:create', (_event, input: CreateCommentInput) => {
    const id = uuidv4();
  ...
  ipcMain.handle('comments:update', (_event, id: string, updates: UpdateCommentInput) => {
    const sets: string[] = [];
